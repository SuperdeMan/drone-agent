...
from a labelled fixture file and must never be reported as model behaviour; `none` refuses to plan. A
missing key is reported, never replaced by a mock (D029).
...
拒绝规划。缺少密钥如实报告，绝不以 mock 代替（D029）。
"""
...
def build_planner(args, registry: Registry):
    ...
    if args.planner == "scripted":
        provider = KeyedScriptedProvider.load(args.fixtures)
...
def main() -> None:
    ...
    parser.add_argument("--planner", choices=["live", "scripted", "none"], default="live")

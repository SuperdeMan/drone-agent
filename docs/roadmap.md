...
| P0 | 2026-09-25 完成，软件 / SITL 范围 | `de597d0`：1000 项云端检查、18/18 E2E、18 份来源视图与两次 MiniMax-M3 实调探针，七项门禁全通过；[记录](p0-readiness.md) |
| P1 | 2026-09-26 完成，软件 / SITL 范围 | `b49701b`：1102 项云端检查；S0 故障矩阵 42/42（3 站 / 3 逻辑机场 / 3 逻辑 UAV，P1-F01–F14 × 3 种子）；S1 5/5（PX4 SITL 上的 `uav_01` 经虚拟机场）；M2 18/18；常驻任务台带 P1 目录与已迁移账本；九项门禁全通过，[记录](p1-readiness.md) |
| P2 | 2026-09-26 完成，软件 / SITL 范围 | `3bdbd50`：1174 项云端检查；S0 故障矩阵 45/45（P2-F01–F15 × 3 种子）与 P1 S0 回归 42/42；S1 5/5（PX4 SITL 上的 `uav_01` 经虚拟机场，8 次飞行，含 3 条到模拟工单与复检的业务链）；M2 18/18；常驻任务台带工作流目录与已迁移账本；十项门禁全通过，[记录](p2-readiness.md) |
| P3–P5、H1–H3、X1–X3 | 待实施 / 待对应条件 | 下表和任务计划定义未来交付，不代表已运行 |
...
|---|---|---|---|
| **P0 范围与证据基线** | 当前能力盘点、来源契约、旧任务映射、产品门禁 | 设计 / 实现 / 仿真 / 硬件状态明确；运行来源贯穿服务与证据；历史结果不回填；所需门禁按准确候选检查 | WP-P0-01–04 已完成；[验收](p0-readiness.md)，[任务](operations-implementation.md) |
...
| **P3 多站多机任务调度** | 硬约束过滤、确定性排序、任务所有权、时空预约、故障对账、任务级接力 | S0 10/30/100 逻辑节点阶梯；至少 2 台同世界 S1 ×3 种子；双重所有权、重复执行、丢任务、冲突预约、假成功均 0 | P2；WP-P3-01–06，[P3 详细方案](p3-implementation.md)（D059–D061，存储扩展 D060 已批准，实施中）；3 台 S1 为容量评估后扩展 |
...
- [x] WP-P0-03：来源对象与贯穿服务 / 证据的记录，已按 `de597d0` 验收。
- [x] WP-P0-04：产品门禁与能力清单，保留历史 M3 子结论 / 总结论区分，已验收。
- [x] WP-P1-01 / 02：资源契约与持久化设计（D055、D056，schema 变更经用户批准）。
- [x] WP-P1-03–08：存储、单机场模拟、正式服务接入与单机 SITL 链，已按 `b49701b` 验收。
...
- [ ] WP-P3-01 / 02 与 WP-P4-01：两机容量测量与确定性分配、主场景数据与指标冻结，可并行。

按[近期批次](operations-implementation.md)领取 P3 / P4 任务；P2 的接手物见 [P2 记录](p2-readiness.md)。每阶段更新入口、对应 readiness 和路线状态；只有实际运行才向 `eval/BASELINES.md` 追加结果。季度技术雷达继续保留。

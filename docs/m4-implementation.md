...
前置：M3 已关闭。A 线同时受采购、场地与报备周期约束，硬件选型与实名登记应在 M3 期间提前启动（批次 A0）。
...
A 线每次飞行记：检查单、报备记录、飞控参数哈希、平台配置哈希、软件 SHA、ULog、机载账本、影像、现场观察、气象、参与人员角色；共因故障每类记注入方式（物理断电 / 拔线 / RC 切换 / 飞控参数触发）、飞控行为与时间线。B 线沿用 M1 要求，另记两机真值、交接事件时间线、对齐前后协方差、预约表。

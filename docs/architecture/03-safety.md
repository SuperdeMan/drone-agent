...
- **CBF 约束过滤**：每条授权前把期望速度投影到安全集合（登记围栏、按半径膨胀的登记障碍、新鲜局部地图的邻近障碍点），不可行即拒绝并恢复；M1 的包络与前瞻检查保留为下界。局部地图过期视同观测过期。
...
- **自主层与学习策略**：规划节点只提交候选片段，与学习型插件走同一条过滤路径；影子阶段的输出只进记录，从不连接 guardian。事件检测只产生候选事实，控制路径不依赖它，进程被杀任务照常执行。

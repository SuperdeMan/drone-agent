...
P3 已实现的部分（D059–D061）：云端 S1 用独立的 `compose.p3.yaml`（不叠加 M2 文件）与两机仿真镜像 `sim/p3.Dockerfile`（基于 aircraft 镜像，M1 / M2 镜像与世界不变）；一个 `sitl-p3` 容器内一个 gz 服务与 PX4 实例 0 / 1，两个 guardian 以固定地址接入内部网络 `sim_p3`（网段由执行器从空闲私网段中选取），每机独立的 uplink（第二台用 `robot-tls-uav_02` 证书）、executive、真值 / 相机转接与逻辑机场后端。执行器在用例期间停止本项目空闲 SITL，经服务容器内的常驻 API 中继调用服务，并以 cgroup 采样记录容量证据；争用时仿真器、guardian 与转接容器的 CPU 权重更高，上限不变。常驻任务台的服务另带 `p3_desk_v1` 调度目录，激活前在账本副本上依次演练 D056、D058 与 D060，服务启动时带备份加上调度表。

S0/S3 运行环境不接真实设备、真实凭据或生产队列；S2 素材回放不触发飞行。RunProvenance 绑定受信后端、媒体与模型来源，不由网页传入的标签决定。四层定义与可证明范围见 [roadmap](../roadmap.md)。

P1 起常驻任务台多一个无网络的 `desk-dock` 逻辑机场后端，服务带 `p1_s1_v1` 目录与 secrets 中的成员列表启动，账本按 D056 带备份迁移到 schema v2（激活前先在副本上演练）；云端 S1 用例用 `compose.p1.yaml` 叠加在 M2 用例文件上。运营持久化优先评估 SQLite 与现有媒体文件机制；具体 schema / 迁移、备份与失败恢复方案须先完成并按项目红线批准后实施。新服务命令、网络、配置与回执只在相应工作包实现后补入 [云端操作指南](../cloud-development.md)，本节不是可直接执行的部署步骤。

...
P2 起常驻任务台另带工作流目录 `p2_s1_v1`（服务参数 `--workflows`）。激活时先在账本副本上依次演练 P1 与 P2 迁移，再备份并迁移真实账本（D058）；迁移后 P1 版本仍能以该账本启动。`desk-members --apply` 给本机 tailnet 身份加上 `campus_s1` 的 reviewer 角色。

1. **模板**：「工作流」面板列出项目的模板（`asset_check`、`campus_round`、`asset_reinspection`）与各触发。每日排班默认停用，operator 可启停并写明原因。
2. **启动**：选择模板与资产后启动。运行时间线显示每个节点的状态、等待原因（审批、设备、环境、投递、飞行、证据复核、人工复核、维修反馈）与失败原因，并链接子任务。
3. **审批**：每个子任务仍在原任务视图按任务包哈希逐个审批；工作流没有批量审批。
4. **复核与工单**：分析结果标注来源（脚本 / 确定性）。疑似时等待 reviewer 确认或排除；确认后建模拟工单。operator 报告维修后，工单转为待复检并启动复检运行（新任务、新飞行、新分析）。P2 不关单。
5. **取消**：取消运行后不再开始新节点；已领取或在飞的子任务按 P1 规则取消与对账，释放前运行显示「取消中」。
6. **草案**：输入自然语言可得到一份模板草案预览（模型只填窄意图，代码编译校验）。草案不保存、不能启动或激活，生效必须经版本化配置评审。

`desk_probe.py workflow --origin <origin> --draft-text "<描述>"` 记录模板、排班、草案与对注入 / 控制 / 激活帧的拒绝。加 `--start` 时按页面方式启动 `asset_check`、逐个审批、复核确认并反馈维修，直到复检运行完成且每次飞行都有裁判结果。当前版本与验收见 [P2 记录](p2-readiness.md)。

## P3 调度入口（D059 / D060）

P3 起常驻任务台另带调度目录 `p3_desk_v1`（服务参数 `--scheduling`；站点 `site_s1` 位于共享坐标原点，未领取的分配一分钟后才撤回）。激活时在账本副本上依次演练 P1、P2 与 P3 迁移，再备份并迁移真实账本（D060）。

1. **提交**：「调度」面板只在调度目录为项目的所有站点给出位置时出现。operator 选择资产与优先级提交任务单，不选机器人：由调度器按固定排序（预计到场、近期使用、机器人 ID）确定性选择，viewer 只能查看。
2. **队列与原因**：队列列出每个任务单的状态；排队中的任务单列出每个候选的等待原因（如补能中、航迹单元被占用、落入失联包络），拒绝的任务单给出永久原因（如该机站点未登记此资产）。机器人区显示各机的预览判定与原因、当前分配与代次；空域区显示被持有的航迹单元与失联包络。
3. **详情**：任务单详情列出每次判定（按录制快照可重放，逐候选给出判定、预计到场与原因）与每次分配（代次递增；撤回只发生在领取之前）。
4. **审批**：被分配的任务在原任务视图按任务包哈希逐个审批；调度入口没有审批或批量审批。
5. **取消**：operator 可取消任务单；已领取或在飞的任务按 P1 规则取消与对账，对账后才显示已取消。

`desk_probe.py tasks --origin <origin>` 记录项目是否可调度、队列、各机判定与原因、可选资产、空域持有，以及对注入 / 控制 / 分配帧的拒绝；加 `--start` 时按页面方式提交一个任务单并审批其被分配的任务，直到任务单终结且其飞行有裁判结果。当前版本与验收见 [P3 方案](p3-implementation.md)。

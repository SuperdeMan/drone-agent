...
8. **计算过载注入**：在自主层容器里运行 CPU 竞争进程时，它与三个节点进程平分 0.6 核配额，要么所有节点一起变慢（先触发定位或地图过期），要么都不越限，无法稳定地只让规划越限。`ext_compute_overload` 改为让规划节点自身每周期额外消耗 320 ms CPU（仍在自主层容器配额内争用），规划周期真实越过 250 ms 而地图与定位保持新鲜，正是 D042 所定义的「新鲜但越限的片段」；guardian 周期不受影响由该场景与 D040 推理满载档共同证明。

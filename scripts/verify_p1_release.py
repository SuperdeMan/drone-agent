"""P1 gate: resources, virtual docks and the claim gate on one immutable candidate (WP-P1-12, D055, D056).

Criteria, all bound to the same full commit:
  scope          — changes since the P0 candidate stay in P1 paths; if the onboard runtime, wire, skills, platform or
  ...
  adversarial    — the M2 deterministic and scripted natural-language corpora still authorize nothing
  s0_matrix      — run here, on the clean candidate: P1-F01..F14 x 3 seeds in the S0 world with the independent judge
                   online and from the recordings; wrong dispatch, duplicate dispatch, wrong release, project escape
                   and false success all 0; host stalls may void an attempt, never pass one
  s1             — remote_p1 receipts: every S1 case x seed passed its judge online and from the recordings with the
  ...
  desk           — the resident desk activated on this candidate with the P1 catalog, a passed migration drill and
                   the real migration; the page, script and health match; the resource entry shows projects,
                   ages, sources and eligibility apart from the link, and refuses injection or control frames
  desk_session   — one mission through the desk's project binding completed and passed the supervisor's judge
  historical     — the M3 and P0 release records are byte-identical to their closing commits
A criterion without evidence is `missing`, never `passed`; S0 and S1 are counted separately and never as physical
flights of three aircraft.

P1 门禁：在同一不可变候选上核对资源、虚拟机场与领取闸门（WP-P1-12，D055，D056）。判据全部绑定同一完整提交：
scope（相对 P0 候选的改动留在 P1 路径内；若机载运行时、wire、技能、平台或恢复策略改变，则必须补 M1 回归，这里不豁免）、
checks（云端部署回执：Linux 全量 0 失败 0 错误 0 跳过，并复核契约字段与 v1 wire 冻结）、adversarial（M2 语料仍不授权任何
任务包）、s0_matrix（在干净候选上当场运行：S0 世界中 P1-F01..F14 × 3 种子，独立裁判在线与按录制各判一次；错误派遣、重复
派遣、错误释放、项目越权与错误成功全部为 0；主机停顿只能使一次尝试作废，不能使其通过）、s1（remote_p1 回执：每个 S1 用例
× 种子在线与回放都通过，计数全为 0，其他容器不变）、m2_regression（服务已改，M2 端到端 18 例在本候选通过）、desk（常驻任务
台以 P1 目录在本候选激活，迁移演练通过且实际迁移成功；页面、脚本与健康一致；资源入口显示项目、年龄、来源以及与链路分开
的可派遣判定，并拒绝注入或控制帧）、desk_session（一次经任务台项目绑定的任务完成并通过监管者的裁判）、historical（M3 与
P0 的发布记录与其关闭提交逐字节一致）。没有证据的判据是 `missing`，绝不是 `passed`；S0 与 S1 分开计数，从不当作三架
飞行器的物理飞行。
"""
...
P0_SHA = "de597d0eb7430260285e0965dc07f7f904ab5e70"
P0_CLOSE = "6d60f67"
...
ONBOARD = ("src/drone_agent/mission/", "src/drone_agent/guardian/", "src/drone_agent/adapters/",
           ...
           "configs/platforms/", "configs/recovery_policies/", "sim/m1.Dockerfile", *(
               f"src/drone_agent/runtime/{name}" for name in ("launch.py", "ipc.py", "ledger.py", "wire.py",
                                                             ...
                                                             "uplink.py")))
P1_PATHS = ("src/drone_agent/fleet/", "src/drone_agent/console/", "src/drone_agent/eval/",
            "src/drone_agent/runtime/permission.py", "src/drone_agent/runtime/issues.py", "configs/sites/",
            "configs/scenarios/p1_", "scripts/", "sim/compose.p1.yaml", "sim/compose.desk.yaml", "tests/")
COUNTS = ("wrong_dispatch", "duplicate_dispatch", "wrong_release", "project_escape", "false_success")
...
def scope(sha: str, m1: list[dict]) -> dict:
    if subprocess.run(["git", "merge-base", "--is-ancestor", P0_SHA, sha], cwd=ROOT).returncode:
        return {"status": "failed", "reason": "candidate does not descend from the P0 candidate"}
    changed = git("diff", "--name-only", P0_SHA, sha).splitlines()
    outside = [p for p in changed if not p.startswith((*DOCS, *P1_PATHS, *ONBOARD))]
    onboard = [p for p in changed if p.startswith(ONBOARD)]
    result = {"base_sha": P0_SHA, "changed": len(changed), "outside_p1_scope": outside, "onboard_changed": onboard}
    if outside:
...
def s0_matrix(sha: str, output: Path) -> dict:
    """Run the S0 matrix here on the clean candidate and summarize it. / 在干净候选上当场运行 S0 矩阵并汇总。"""
    ...
    if any(counts.get(k) for k in COUNTS):
...
def s1(receipts: list[dict], sha: str) -> dict:
    ...
    suite = yaml.safe_load(subprocess.check_output(["git", "show", f"{sha}:configs/scenarios/p1_suite.yaml"],
...
def desk(activation: dict | None, http: dict | None, probe: dict | None, sha: str) -> dict:
    if activation is None or http is None or probe is None:
        return {"status": "missing", "reason": "desk activation, HTTP and resource probe receipts are required"}
    problems = []
    operations = activation.get("operations") or {}
    migration = operations.get("migration") or {}
    drill = operations.get("drill") or {}
    if activation.get("status") != "verified" or activation.get("source_sha") != sha:
        problems.append("desk_not_activated_on_candidate")
    if operations.get("catalog_id") != "p1_s1_v1" or migration.get("status") not in ("migrated", "current"):
        problems.append("desk_without_p1_catalog_or_migration")
    if drill.get("status") not in ("passed", "not_applicable"):
        problems.append("migration_drill_not_passed")
    ...
    projects = {p["project_id"]: p for p in probe.get("projects", [])}
    campus = projects.get("campus_s1") or {}
    if not {"operator", "approver", "admin"} <= set(campus.get("roles") or []) or campus.get("robots") != ["uav_01"]:
        problems.append("operator_project_roles_missing")
    view = (probe.get("resources") or {}).get("campus_s1") or {}
    docks, robots = view.get("docks") or [], view.get("robots") or []
    if not docks or docks[0]["source"] != "logical_sim" or not docks[0].get("status") \
            or docks[0]["status"].get("age_s") is None or docks[0]["status"].get("session") is None:
        problems.append("dock_status_age_source_or_session_missing")
    if not robots or robots[0].get("eligibility", {}).get("verdict") not in ("eligible", "blocked", "unknown") \
            or "reasons" not in robots[0].get("eligibility", {}):
        problems.append("robot_eligibility_missing")
    refused = probe.get("refused") or []
    if len(refused) < 5 or any("unknown type" not in (r.get("error") or "") for r in refused):
        problems.append("injection_or_control_frames_not_refused")
    if (probe.get("script") or {}).get("control_or_injection_frames"):
        problems.append("page_script_has_control_or_injection_frames")
    return {"status": "failed" if problems else "passed", "problems": problems,
            "catalog_id": operations.get("catalog_id"), "migration": migration.get("status"),
            "drill": drill.get("status"), "dock_pad": docks[0].get("pad") if docks else None}


def desk_session(receipt: dict | None, flights: list[dict] | None, sha: str) -> dict:
    if receipt is None or flights is None:
        return {"status": "missing", "reason": "a desk mission through the project binding and its flight.json"}
    ...
    if (binding.get("project_id"), binding.get("robot_id"), binding.get("dock_id")) != ("campus_s1", "uav_01", "dock_s1"):
        problems.append("mission_not_bound_to_the_p1_site")
    claims = ((mission.get("dispatch") or {}).get("claims")) or []
    reservations = ((mission.get("dispatch") or {}).get("reservations")) or []
    if not claims or any(c.get("state") != "claimed" for c in claims):
        problems.append("delivery_not_claimed_through_the_gate")
    if not reservations or any(r.get("state") != "released" or r.get("reason") != "reconciled" for r in reservations):
        problems.append("hold_not_released_by_reconciliation")
    if not (mission.get("status") == "completed" and judge.get("passed") is True and judge.get("replay_agrees") is True
            and judge.get("false_success_reports") == 0 and judge.get("problems") == []):
        problems.append("mission_not_independently_verified")
    public = (mission.get("cloud") or {}).get("flights") or []
    recorded = {(f.get("mission_id"), f.get("version")): f for f in flights}
    for flight in public:
        own = recorded.get((mission.get("mission_id"), flight.get("version")))
    ...
    if not public:
        problems.append("no_desk_flight")
    return {"status": "failed" if problems else "passed", "problems": problems,
            "mission_status": mission.get("status"), "flights": len(public)}


def historical() -> dict:
    """Closed milestones' release records are inputs, never current results. / 已关闭里程碑的记录是输入，不是当前结果。"""
    result, problems = {}, []
    for label, path, commit in (("m3", M3_PATH, "9223d74"), ("p0", P0_PATH, P0_CLOSE)):
    ...
    if result["p0"]["status"] != "passed":
        problems.append("p0_not_closed")
...
def capabilities(sha: str, criteria: dict) -> list[dict]:
    ...
    return [
        {"capability": "resources_virtual_docks_claim_gate", "milestone": "P1", "implemented": True,
         "software_validation": "passed" if passed else "not_passed", "source_sha": sha,
         "layers": {"S0": {"sites": 3, "docks": 3, "logical_uavs": 3, "isolation_fixture_project": 1,
                           "physical_flight": False},
                    "S1": {"px4_sitl_aircraft": 1, "logical_docks": 1}},
         "hardware_validation": "not_applicable"},
        {"milestone": "P0", "software_validation": "historical_passed", "source_sha": P0_SHA,
         "counts_as_candidate_validation": False},
        {"milestone": "M3", "status": "not_passed", "reason": "JIL missing; not a P1 dependency"},
        ...
        *({"milestone": name, "status": "planned", "software_validation": "missing"}
          for name in ("P2", "P3", "P4", "P5", "X1", "X2", "X3")),
    ]
...
def main() -> None:
    ...
    parser.add_argument("--desk-resources", type=Path, help="desk_probe.py resources receipt")
    parser.add_argument("--desk-session", type=Path, help="desk_probe.py session receipt through the P1 binding")
    parser.add_argument("--desk-flights-dir", type=Path, help="the supervisor's flight.json for that session")
    parser.add_argument("--s0-output", type=Path, default=ROOT / "outputs" / "p1-s0")
    ...
    inputs = [p for p in [args.deployment, args.desk, args.desk_http, args.desk_resources, args.desk_session,
    ...
    criteria = {
        ...
        "desk": desk(load(args.desk), load(args.desk_http), load(args.desk_resources), args.sha),
    ...
    report = {"schema_version": "0.1.0", "milestone": "P1", "source_sha": args.sha, "records_sha": head,
